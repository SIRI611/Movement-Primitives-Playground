"""
Probabilistic Movement Primitives (ProMP) implementation.

ProMPs extend DMPs by incorporating probabilistic modeling to handle
variations in demonstrations and enable probabilistic inference.
"""
...
@dataclass
class ProMPConfig(MovementPrimitiveConfig):
    """Configuration for Probabilistic Movement Primitives."""
    
    # ProMP-specific parameters
...
class ProMP(BaseMovementPrimitive):
    """
    Probabilistic Movement Primitives implementation.
    
    ProMPs model movement primitives as Gaussian distributions over
    weight parameters, enabling probabilistic inference and adaptation.
    """
    
    def __init__(self, config: ProMPConfig):
        """
        Initialize ProMP.
        
        Args:
            config: ProMP configuration
        """
        super().__init__(config)
        self.config: ProMPConfig = config
    ...
    def fit(self, trajectories: Union[np.ndarray, List[np.ndarray]], 
            **kwargs) -> 'ProMP':
        """
        Train ProMP on demonstration trajectories.
    ...
    def generate(self, start: Optional[np.ndarray] = None,
                 ...
                 **kwargs) -> np.ndarray:
        """
        Generate trajectory using trained ProMP.
        ...
        if not self.is_trained:
            raise RuntimeError("ProMP must be trained before generating trajectories")
    ...
    def get_weights(self) -> np.ndarray:
        ...
        if not self.is_trained:
            raise RuntimeError("ProMP must be trained before accessing weights")
    ...
    def get_weight_distribution(self) -> Tuple[np.ndarray, np.ndarray]:
        ...
        """
        if not self.is_trained:
            raise RuntimeError("ProMP must be trained before accessing weight distribution")
    ...
    def condition_on_waypoint(self, waypoint: np.ndarray, 
                             waypoint_time: float,
                             execution_time: Optional[float] = None) -> 'ProMP':
        """
        Condition the ProMP on a waypoint.
        ...
        """
        if not self.is_trained:
            raise RuntimeError("ProMP must be trained before conditioning")
        ...
        # Create conditioned ProMP
        conditioned_promp = ProMP(self.config)
        conditioned_promp.weight_mean = self.weight_mean.copy()
        conditioned_promp.weight_cov = self.weight_cov.copy()
        conditioned_promp.is_trained = True
        
        # Condition on waypoint for each dimension
        for dim in range(self.config.n_dims):
            conditioned_promp.weight_mean[dim], conditioned_promp.weight_cov[dim] = \
                self._condition_dimension_on_waypoint(dim, waypoint[dim], waypoint_idx, phase)
        
        return conditioned_promp
    ...
    def get_trajectory_variance(self, execution_time: Optional[float] = None) -> np.ndarray:
        ...
        """
        if not self.is_trained:
            raise RuntimeError("ProMP must be trained before computing trajectory variance")
    ...
    def sample_trajectories(self, n_samples: int, 
        ...
        """
        Sample multiple trajectories from the ProMP.
